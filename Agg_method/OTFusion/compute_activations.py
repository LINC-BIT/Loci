...
import os
import Agg.OTFusion.utils as myutils
import sys
PATH_TO_CIFAR = "./cifar/"
...
def compute_activations_across_models(args, models, train_loader, num_samples, dump_activations=False, dump_path=None):
    ...
    for batch_idx, data in enumerate(train_loader):
        if args.server_gpu != -1:
            data = data.cuda(args.server_gpu)
        for idx, model in enumerate(models):
            model(data)
        num_samples_processed += 1
        if num_samples_processed == num_samples:
...
def compute_activations_across_models_v1(args, models, train_loader, num_samples, mode='mean',
    ...
    def get_activation(activation, name):
        def hook(model, input, output):
            ...
            if name not in activation:
                activation[name] = []
            chunks = torch.chunk(output.detach(), output.size(0), dim=0)
            for c in chunks:
                activation[name].append(c.detach())
