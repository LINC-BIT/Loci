...
class Appr():
    ...
    def train_kd(self,t,tr_dataloader):
        cur_kd = self.cur_kd
        cur_kd.train()
        l, a = self.validKD(t, tr_dataloader,model=cur_kd)
        print('first kd: ',a)
        kd_optimizer = torch.optim.Adam(cur_kd.parameters(), lr=0.001)
    ...
    def observe(self, x, t, y,kd_lambda = 0):
        ...
        kd_output = cur_kd(x,t)
        cur_output = self.model.forward(x, t)[:, offset1: offset2]
        loss = self.ce(cur_output, y) + kd_lambda * MultiClassCrossEntropy(cur_output,kd_output,t,T=2)
    ...
    def validKD(self,t,tr_dataloader,model=None):
        total_loss = 0
        total_acc = 0
        total_num = 0
        if model is None:
    ...
    def get_no_bias_dict(self,state_dict = None):
        if state_dict is None:
            state_dict = self.cur_kd.state_dict()
        no_bias_dict = collections.OrderedDict()
        for k in state_dict.keys():
            if 'bias' not in k:
                no_bias_dict[k] = state_dict[k]
        return no_bias_dict
    def prune_kd_model(self,dataloader,task):
        # prune_kd_state_dict = self.model_prune.prune_one_model(self.cur_kd,dataloader,task)
        # l, a = self.validTest(task, dataloader, model=self.cur_kd)
        # print('tune kd: ', a)
        self.train_kd(task, dataloader)

        kd_state_dict = self.cur_kd.state_dict()
        no_bias_dict = self.get_no_bias_dict(kd_state_dict)
        return no_bias_dict
...
def LongLifeTrain(args, appr, tr_dataloader, aggNum,idx):
    ...
    # Train
    loss = life_experience(task,appr,tr_dataloader,5,args.local_bs,args)
    print('-' * 100)
    return appr.get_no_bias_dict(), loss, 0
