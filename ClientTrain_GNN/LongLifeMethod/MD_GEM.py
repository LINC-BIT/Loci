...
class Appr():
    ...
    def set_pubData(self,pubData):
        self.pub_dataloader = pubData
    ...
    def set_trData(self,dataloader):
        pass
    def observekd(self,images,task,targets):
        cur_kd = self.cur_kd
        cur_kd.train()
        ...
        outputs = cur_kd.forward(images, task)
        loss = self.ce(outputs, targets)
        kd_optimizer.zero_grad()
        loss.backward()
        kd_optimizer.step()
    def train_pub(self,t):
        self.opt = optim.Adam(self.model.parameters(), self.lr)
        self.model.train()
        # Forward current model
        for epoch in range(self.pub_epoch):
            for images,targets in self.pub_dataloader:
                images = images.to(self.device)
                targets = (targets).to(self.device)
                offset1, offset2 = compute_offsets(t, self.nc_per_task)
                outputs = self.model.forward(images, t)[:, offset1:offset2]
                label_loss = self.ce(outputs, targets)
                kd_loss = 0
                if self.global_act is not None:
                    kd_loss = MultiClassCrossEntropy(outputs,self.global_act,t)
    ...
    def observe(self, x, t, y,kd_lambda = 0):
        ...
        loss = self.ce(cur_output, y) + kd_lambda * self.softloss(cur_output,kd_output)
...
def life_experience(task,appr,tr_dataloader,epochs,sbatch=10,args=None):
    ...
    for e in range(epochs):
        for images,targets in tr_dataloader:
            ...
            appr.observekd(images, task,targets)
