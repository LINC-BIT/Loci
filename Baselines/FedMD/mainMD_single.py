...
if __name__ == '__main__':
    ...
    print(args.alg)
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrain/log/single/'+args.clmethod+'/'+args.clmethod+'localall_server_epoch10_high20_dag_' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
    # net_glob = get_model(args)
    net_glob = SixCNN([3,32,32],outputsize=100)
    net_glob.train()
    net_glob_cg = ChannelGatedCL(in_ch=cfg.IN_CH, out_dim=cfg.OUT_DIM,
                              conv_ch=cfg.CONV_CH,
    ...
    if args.clmethod == 'EWC':
        ...
        apprs = [Packnet_Appr(copy.deepcopy(net_glob).to(args.device), None,lr=args.lr, nepochs=args.local_ep, args=args,kd_model=kd_model) for i in range(args.num_users)]
    elif args.clmethod == 'ChannelGate':
        apprs = [ChannelGate_Appr(copy.deepcopy(net_glob_cg).to(args.device), None,lr=args.lr, nepochs=args.local_ep, args=args,kd_model=kd_model) for i in range(args.num_users)]
    else:
    ...
    for iter in range(args.epochs):
        ...
        if args.clmethod == 'Packnet':
            ...
            acc_test, loss_test = test_img_local_all(None, args, dataset_test, dict_users_test, task, apprs=apprs,
                                                     ...
                                                     client_task=client_task, spe=False)
