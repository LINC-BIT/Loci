...
def train_step(model, kd_model, optimizer, scheduler, epoch_num, train_loader, logger=None, kd_lambda=0.0, t = 0):
    ...
    for batch in train_loader:
        ...
        if kd_model is not None:
            ...
            kd_loss = kd_lambda * softloss(out, kd_output)
...
def train_pub(model,optimizer,pub_epoch,pub_dataloader,t,global_act,device):
    model.train()
    # Forward current model
    for epoch in range(pub_epoch):
        for images, targets in pub_dataloader:
            images = images.to(device)
            targets = (targets).to(device)
            head_idx = torch.full_like(targets, t).to(device)
            outputs = model(images, head_idx)
            label_loss = F.cross_entropy(outputs, targets)
            kd_loss = 0
            if global_act is not None:
                kd_loss = MultiClassCrossEntropy(outputs, global_act, t)
