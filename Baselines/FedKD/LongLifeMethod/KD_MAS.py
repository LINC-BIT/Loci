...
class Appr(object):
    ...
    def train(self, t):
        if t!=self.old_task:
            ...
            self.model_old = deepcopy(self.model)
            if self.old_task>=0:
                self.omega_update(self.old_task)
            self.old_task=t

            self.first_train = True
        self.kd_optimizer  = torch.optim.Adam(self.model.parameters(), lr=0.0005)
        lr = self.lr
        ...
        kd_grad = []
        for parameter in self.cur_kd.parameters():
            kd_grad.append(parameter.grad.data)

        return kd_grad,train_loss, train_acc
    ...
    def train_kd(self,images,targets,t):
        ...
        loss = self.ce(kd_outputs, targets) + self.softloss(kd_outputs,outputs)
    ...
    def train_epoch(self, t, epoch):
        ...
        for images, targets in self.tr_dataloader:
            ...
            # kd_outpus, kd_hiddens = self.cur_kd.forward(images, t)
            # kd_outpus = kd_outpus[:, offset1:offset2]
            # outputs, model_hiddens = self.model.forward(images, t)
            # outputs = outputs[:, offset1:offset2]


            kd_outpus = self.cur_kd.forward(images, t)
            kd_outpus = kd_outpus[:, offset1:offset2]
            ...
            model_label_loss = self.criterion(t, outputs, targets)
            kd_label_loss = self.ce(kd_outpus, targets)
        ...
        for images,targets in self.tr_dataloader:
            ...
            for n, p in self.model.named_parameters():
                if p.grad is not None:
                    self.omega[n] += p.grad.data.abs() / len(self.tr_dataloader.dataset)
...
def LongLifeTrain(args, appr, aggNum, writer,idx):
    ...
    # Train
    kd_grad,loss,_ = appr.train(task)
    print('-' * 100)
