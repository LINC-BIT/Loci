...
class Appr():
    ...
    def train_kd(self,t,tr_dataloader):
        ...
        for e in range(self.kd_epoch):
            for images, targets in tr_dataloader:
                ...
                outputs = cur_kd.forward(images, t)[:, offset1:offset2]
    ...
    def observe(self, x, t, y,kd_lambda = 0):
        ...
        kd_outpus = kd_outpus[:, offset1:offset2]
        ...
        model_label_loss = self.ce(outputs, y - offset1)
        kd_label_loss = self.ce(kd_outpus, y - offset1)
...
def life_experience(task,appr,tr_dataloader,epochs,sbatch=10,args=None):
    ...
    kd_lambda = appr.train_kd(task, tr_dataloader)
    for e in range(epochs):
        for images,targets in tr_dataloader:
            images = images.to(args.device)
            targets = targets.to(args.device)
            appr.model.train()
            appr.observe(images, task, targets, kd_lambda)
