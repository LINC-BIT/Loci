...
def train_kd(cur_kd,model, images, targets, t,kd_lambda):
    cur_kd.train()
    ...
    kd_outputs = cur_kd.forward(images, t)[:, offset1:offset2]
    head_idx = torch.full_like(targets, t).to(cfg.DEVICE)
    outputs = model(images, head_idx)
    loss = ce(kd_outputs, targets) + kd_lambda*softloss(kd_outputs, outputs)
    loss.backward()
...
def train_step(model, kd_model, optimizer, scheduler, epoch_num, train_loader, logger=None, kd_lambda=0.0, t = 0,):
    ...
    for batch in train_loader:
        ...
        if kd_model is not None:
            offset1, offset2 = compute_offsets(t, 10)
            kd_output = kd_model(x, t)[:, offset1:offset2]
            lamb =  1/(float(F.cross_entropy(kd_output, y)) + float(head_loss))
            kd_loss =lamb* kd_lambda*softloss(out,kd_output)
        ...
        train_kd(kd_model,model,x,y,t,kd_lambda*lamb)
