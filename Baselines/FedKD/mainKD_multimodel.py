...
from ClientTrainFedKD.LongLifeMethod.KD_EWC import Appr as EWC_Appr
from ClientTrainFedKD.LongLifeMethod.KD_EWC import LongLifeTrain as EWC_LongLifeTrain
from ClientTrainFedKD.LongLifeMethod.KD_GEM import Appr as GEM_Appr
from ClientTrainFedKD.LongLifeMethod.KD_GEM import LongLifeTrain as GEM_LongLifeTrain
from ClientTrainFedKD.LongLifeMethod.KD_FedKNOW import Appr as FedKNOW_Appr
from ClientTrainFedKD.LongLifeMethod.KD_FedKNOW import LongLifeTrain as FedKNOW_LongLifeTrain
from ClientTrainFedKD.LongLifeMethod.KD_MAS import Appr as MAS_Appr
from ClientTrainFedKD.LongLifeMethod.KD_MAS import LongLifeTrain as MAS_LongLifeTrain
from ClientTrainFedKD.LongLifeMethod.KD_Packnet import Appr as Packnet_Appr
from ClientTrainFedKD.LongLifeMethod.KD_Packnet import LongLifeTrain as Packnet_LongLifeTrain
from ClientTrainFedKD.LongLifeMethod.KD_ChannelGate import Appr as ChannelGate_Appr
from ClientTrainFedKD.LongLifeMethod.KD_ChannelGate import LongLifeTrain as ChannelGate_LongLifeTrain
from ClientTrain.AggModel.sixcnn import SixCNN
from torch.utils.data import DataLoader
from Agg.AggModel.sixcnn import SixCNN as KDmodel
import time
from Agg.FedKD import FedKD
...
if __name__ == '__main__':
    ...
    print(args.alg)
    write = SummaryWriter(
        '/data/lpyx/FedAgg/ClientTrainFedKD/log/multi/' + args.clmethod + '/' + args.clmethod + 'local5_lastall_epoch10_high20_dag_' + args.dataset + '_' + 'round' + str(
            args.round) + '_frac' + str(args.frac))
    # build model
    ...
    for idx in range(args.num_users):
        if multi_model_dict[idx]['cl_method'] == 'EWC':
            appr = EWC_Appr(multi_model_dict[idx]['model'].to(args.device), None, lr=multi_model_dict[idx]['lr'],
                            nepochs=1, args=args, kd_model=kd_model)
            ...
            appr = FedKNOW_Appr(multi_model_dict[idx]['model'].to(args.device), PackNet(args.task, device=args.device),
                                ...
                                lr=multi_model_dict[idx]['lr'], nepochs=1, args=args,
                                kd_model=kd_model)
            ...
            appr = ChannelGate_Appr(multi_model_dict[idx]['model'].to(args.device), None,
                                    lr=multi_model_dict[idx]['lr'], nepochs=1,
                                    args=args,
                                    kd_model=kd_model)
        else:
        ...
        for i in range(5):
            for ind, idx in enumerate(idxs_users):
                ...
                if multi_model_dict[idx]['cl_method'] == 'EWC':
                    ...
                    kd_models, loss, indd = FedKNOW_LongLifeTrain(args,appr,iter,None,idx,i)
                elif multi_model_dict[idx]['cl_method'] == 'MAS':
