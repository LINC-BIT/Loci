...
from ClientTrainFedKEMF.LongLifeMethod.KEMF_GEM import Appr,LongLifeTest,LongLifeTrain
...
if __name__ == '__main__':
    ...
    print(args.alg)
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrainFedKEMF/log/GEM/GEM_high_20past_cd ' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
    ...
    start = time.time()
    task=-1
    apprs = [Appr(net_glob.to(args.device), kd_model,3*32*32,100,10, args) for i in range(args.num_users)]
    first_kd_dict=kd_model.state_dict()
    print(args.round)
    w_globals = []
    for iter in range(args.epochs):
        if iter % (args.round) == 0:
            ...
            for i in range(args.num_users):
                apprs[i].kd_model.load_state_dict(first_kd_dict)
        w_glob = {}
        loss_locals = []
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            kd_models,loss, indd = LongLifeTrain(args,appr,tr_dataloaders,iter,idx)
