...
from ClientTrain.models.test import test_img_local_all_channel
from ClientTrainFedKEMF.LongLifeMethod.KEMF_ChannelGate import Appr,LongLifeTrain
...
if __name__ == '__main__':
    ...
    print(args.alg)
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrainFedKEMF/log/ChannelGate/ChannelGate_high20_dag_' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
    # net_glob = get_model(args)
    net_glob = ChannelGatedCL(in_ch=cfg.IN_CH, out_dim=cfg.OUT_DIM,
                           conv_ch=cfg.CONV_CH,
    ...
    print(args.round)
    serverAgg = FedKEMF(int(args.frac * args.num_users),datasize=[3,32,32],dataname='CIFAR100',output=10)
    w_globals = None
    for iter in range(args.epochs):
        ...
        acc_test, loss_test = test_img_local_all_channel(None, args, dataset_test, dict_users_test,task,apprs=apprs,w_locals=None,return_all=False,write=write,round=iter,client_task=client_task)
