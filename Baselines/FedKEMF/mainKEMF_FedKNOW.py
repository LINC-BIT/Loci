...
from ClientTrainFedKEMF.LongLifeMethod.KEMF_FedKNOW import Appr,LongLifeTest,LongLifeTrain
...
if __name__ == '__main__':
    ...
    print(args.alg)
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrainFedKEMF/log/FedKNOW/FedKNOW_server_dag_' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
    ...
    apprs = [Appr(copy.deepcopy(net_glob),PackNet(args.task,device=args.device),copy.deepcopy(net_glob), None,lr=args.lr, nepochs=args.local_ep, args=args,kd_model=kd_model) for i in range(args.num_users)]
    print(args.round)
