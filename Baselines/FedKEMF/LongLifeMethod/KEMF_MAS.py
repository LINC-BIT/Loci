...
class Appr(object):
    ...
    def train_kd(self,images,targets,t):
        ...
        loss = self.ce(kd_outputs, targets) + self.kd_lambda*self.softloss(kd_outputs,outputs)
    ...
    def train_epoch(self, t, epoch):
        ...
        for images,targets in self.tr_dataloader:
            ...
            loss = self.criterion(t, outputs, targets) + self.kd_lambda*self.softloss(outputs,kd_outpus)
            ## 根据这个损失计算梯度，变换此梯度
            # Backward
...
def LongLifeTrain(args, appr, aggNum, writer,idx):
    ...
    return appr.kd_model.state_dict(), loss, 0
