...
class Appr():
    ...
    def observe(self, x, t, y,kd_lambda = 0):
        ...
        kd_output = cur_kd(x,t)[:,offset1:offset2]
        cur_output = self.model.forward(x, t)[:, offset1: offset2]
        loss = self.ce(cur_output, y - offset1) +  self.kd_lambda*self.softloss(cur_output,kd_output)
    ...
    def observekd(self,images,task,targets):
        ...
        kd_optimizer = torch.optim.Adam(cur_kd.parameters(), lr=0.0005)
        images = images.to(self.device)
        targets = (targets - 10 * task).to(self.device)
        # Forward current model
        offset1, offset2 = compute_offsets(task, 10)
        outputs = cur_kd.forward(images, task)[:, offset1:offset2]
        loss = self.ce(outputs, targets)
        kd_optimizer.zero_grad()
        loss.backward()
        kd_optimizer.step()
